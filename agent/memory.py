...
def write_case(g, ans: dict, case: dict):
    ...
    g.upsert_vertex("AgentCase", cid, {"case_ref": ans["case_id"], "status": c["status"], "verdict": c["verdict"], "pattern": c["pattern"], "probability": c["fraud_probability"],
    ...
    for et, (tt, targets) in edges.items():
        for t in targets:
            try: g.upsert_edge("AgentCase", cid, et, tt, str(t))
            except Exception: pass
    try:
        v = llm.embed([f"{c['pattern']} | {c['verdict']} | {c['summary']}"])[0]
        g.upsert_vertex("AgentCase", cid, {"emb": v})
    except Exception:
        pass
